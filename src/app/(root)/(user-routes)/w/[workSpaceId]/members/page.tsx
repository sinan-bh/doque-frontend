...
export default function Page() {
  ...
  const renderContent = () => {
    if (activeTab === 'members') {
      return viewType === 'grid' ? <MembersGrid members={members} /> : <MembersList members={members} />;
    } else {
      return viewType === 'grid' ? <TeamsGrid teams={teams} /> : <TeamsList teams={teams} />;
    }
  };

  return (
    <div className="p-6 bg-gray-100 min-h-screen w-4/5 mx-auto">
      <div className="mb-4">
      ...
      <div className="border-b border-gray-300 mb-4">
        <div className="flex justify-between">
          <div className="flex space-x-4 mt-2">
            <h2
              className={`text-base font-semibold cursor-pointer ${activeTab === 'members' ? 'border-b-4 border-blue-600' : 'text-gray-700'}`}
              onClick={() => handleTabSwitch('members')}
            >
              Members
            </h2>
            <h2
              className={`text-base font-semibold cursor-pointer ${activeTab === 'teams' ? 'border-b-4 border-blue-600' : 'text-gray-700'}`}
          ...
          <div className="flex items-center space-x-4 pb-2">
            ...
            <div className="flex items-center space-x-4">
              <div 
                ...
                title='Grid View'
              >
                <IoGrid className={`text-gray-600 text-xl ${viewType === 'grid' ? 'text-blue-600' : ''}`} />
              </div>
              <div 
                ...
                title='List View'
              >
                <IoList className={`text-gray-600 text-2xl ${viewType === 'list' ? 'text-blue-600' : ''}`} />
              </div>
            </div>
          </div>
        </div>
